...
def compute_lifetime_value(df, df_lines, transformed_sales_filter):
    df_final = df.merge(df_lines, left_on=f'{transformed_sales_filter}_ID', right_on=f'{transformed_sales_filter}_ID')

    cltv_df = df_final.groupby('Customer_ID').agg(
        Age=(f'{transformed_sales_filter}_date', lambda x: (x.max() - x.min()).days),
        Num_transactions=(f'{transformed_sales_filter}_ID', lambda x: len(x)),
        Quantity=('Quantity', lambda x: x.sum()),
        Total_revenue=('Total_price_y', lambda x: x.sum())
    ).query('Quantity > 0')
...
def customer_overview_data_function(rfm, df_sales, df_lines, transformed_sales_filter, show_full_dataframe=False):
    ...
    if not show_full_dataframe:
        customer_id = st.selectbox('Customer', ((merged_df['Customer_ID'].astype(int)).astype(str)
                                                + ' - ' + merged_df['Customer_name']))
        customer_id = int(customer_id.split(' - ')[0])
        merged_df = merged_df[merged_df['Customer_ID'] == customer_id]
...
def get_map(directory, address, customer_id):
    ...
    if directory == 'Ici store':
        display_france_map(customer_zip)
    else:
        display_world_map()


def display_france_map(customer_zip):
    geojson_france = './utils/Geo/department-contours.geojson'
    gdf_departments = gpd.read_file(geojson_france)
    france_map = px.choropleth_mapbox(customer_zip, geojson=gdf_departments, locations='Zip_code',
                                      color='Zip_code', color_continuous_scale='Viridis', range_color=(0, 12),
                                      mapbox_style='carto-positron', zoom=3.5,
                                      center={'lat': 46.6031, 'lon': 1.8883}, opacity=0.8)
...
def customer_overview_main_function(directory, address, rfm, scaler, kmeans, average_clusters,
                                    df_sales, df_lines, transformed_sales_filter):
    cltv_df = compute_lifetime_value(df_sales, df_lines, transformed_sales_filter)
